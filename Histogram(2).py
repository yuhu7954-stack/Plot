...
bars1 = ax1.barh(y - height/2, sim_flow, height, 
                label='RETA', color=colors['simulation'], alpha=0.9, edgecolor='white', linewidth=0.5)
...
# 计算所需的最大x轴范围
max_flow_value = max(max(sim_flow), max(ref_flow))
max_flow_error_text = max([abs(err) for err in flow_abs_errors])
# 设置x轴范围，为误差标注留出空间
x_max_flow = max_flow_value + max_flow_error_text * 0.8
ax1.set_xlim(0, x_max_flow)
...
for i, (abs_err, rel_err) in enumerate(zip(flow_abs_errors, flow_rel_errors)):
    ...
    # 动态调整标注位置，避免超出边界
    text_x = width + max_flow_value * 0.03
    if text_x > x_max_flow * 0.95:  # 如果太靠近边界，向内调整
        text_x = width - max_flow_value * 0.05
...
# 调整布局
fig1.tight_layout()
plt.subplots_adjust(left=0.25)  # 增加左边距确保y轴标签可见
...
# 计算所需的最大x轴范围
max_temp_value = max(max(sim_temp), max(ref_temp))
max_temp_error_text = max([abs(err) for err in temp_abs_errors])
# 设置x轴范围，为误差标注留出空间 - 修改：从500K开始
x_min_temp = 500  # 温度图从500K开始
x_max_temp = max_temp_value + max_temp_error_text * 0.8
ax2.set_xlim(x_min_temp, x_max_temp)
...
for i, (abs_err, rel_err) in enumerate(zip(temp_abs_errors, temp_rel_errors)):
    ...
    # 动态调整标注位置，避免超出边界 - 修改：考虑x_min_temp
    text_x = width + (x_max_temp - x_min_temp) * 0.02
    if text_x > x_max_temp * 0.95:  # 如果太靠近边界，向内调整
        text_x = width - (x_max_temp - x_min_temp) * 0.03
        ha = 'right'
    else:
        ha = 'left'
...
ax2.set_ylabel('Coolant Channels', fontweight='bold')
